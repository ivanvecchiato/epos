<template>
  <div class="produzione">
    <el-row :gutter="20">
      <el-col :span="4"> Nome </el-col>
      <el-col :span="4"> IP stampante </el-col>
    </el-row>
    <ul>
      <li v-for="area in aree" :key="area.id">
        <parametri-area
          :data="area"
          :local="isLocalArea(area)"
          @saveArea="saveArea"
  ...
  methods: {
    ...
    isLocalArea(area) {
      ...
      if(param.id == area.id &&
         ...
         param.printerAddress == area.printerAddress) {
           return true;
         }
      else return false;
    },
    setLocal(obj) {
      localStorage.setItem('local_area', JSON.stringify(obj));
      this.loadAreas();
    },
    removeLocal() {
      localStorage.removeItem('local_area');
    },
    loadAreas() {
      Firebase.db
        .collection("aree")
